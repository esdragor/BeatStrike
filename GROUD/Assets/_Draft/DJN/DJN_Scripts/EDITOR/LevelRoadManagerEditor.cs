...
public class LevelRoadManagerEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < manager.majorSteps.Count; i++)
        {
            if (Handles.Button( manager.majorSteps[i].position + (Vector3.up * 2), Quaternion.identity, 0.3f, 0.5f, Handles.SphereHandleCap))
            {
                SetSelectedStep(manager.majorSteps[i]);
            }
        }
        ...
        GUILayout.BeginArea(toolsArea);
        {
            ...
            manager.stepCount = (int)EditorGUILayout.Slider(manager.stepCount, 1, 500);
            ...
            if (currentStep != null)
            {
                currentStep.stepAction =
                    (LevelRoadManager.RoadStep.StepAction)EditorGUILayout.EnumPopup("Step Action", currentStep.stepAction);
            }
        }
